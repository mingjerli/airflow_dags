...
@dag(
    ...
    dag_id="copy_fraud_detection_model_creation",
)
def fraud_detection_model_creation():
    ...
    def task_get_fraud_detection_data():
        ...
        sql_query = f"SELECT {columns} FROM fraud_detection_features             INNER JOIN fraud_detection_outcome             ON fraud_detection_features.index = fraud_detection_outcome.index             ;"
    ...
    def task_get_fraud_detection_model(data):
        ...
        random_forest_classifier = get_fraud_detection_model(data["X_train"], data["y_train"])
    ...
    def task_get_fraud_detection_predictions(data):
        from fraud_detection_pipeline_module import \
            get_fraud_detection_predictions

        s3hook = S3Hook()
        file_obj = s3hook.get_key(key="random_forest_model", bucket_name="fraud-detection-pipeline-data")
        random_forest_classifier = pickle.loads(file_obj.get()["Body"].read())
        rfc_pred = get_fraud_detection_predictions(random_forest_classifier, data["X_test"])
        return rfc_pred
    ...
    random_forest_classifier = task_get_fraud_detection_model(clean_fraud_detection_data)
    get_fraud_detection_predictions_result = task_get_fraud_detection_predictions(clean_fraud_detection_data)
    get_fraud_detection_performance_report_result = task_get_fraud_detection_performance_report(
